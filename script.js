...
let lastSavedDate = localStorage.getItem("heroTrackLastDate") || new Date().toDateString();
...
function checkAndResetNewDay() {
    ...
    if (lastSavedDate !== today) {
        ...
        lastSavedDate = today;
        localStorage.setItem("heroTrackLastDate", today);
        saveData();
    }
}
...
function renderCards() {
    ...
    if (familyData.length === 0) {
        childrenContainer.innerHTML = `<p class="empty-notice">No heroes registered yet. Type a child's name above and press Enter!</p>`;
        return;
    }
    ...
    familyData.forEach(child => {
        ...
        const dailyColHtml = `
            <div class="daily-col">
                ...
                <button class="btn-reset-day" onclick="resetDay(${child.id})">🔄 Reset Day</button>
            </div>
        `;
            ...
            <div class="weekly-col">
                ...
                <div class="ring-outer" style="background: conic-gradient(#0284c7 ${child.weeklyPct * 3.6}deg, #e2e8f0 0deg)">
                    <div class="ring-inner">
                        <button class="btn-reset-weekly" onclick="resetWeekly(${child.id})">
                            <span>📋</span>
                            <span>Reset</span>
                        </button>
                    </div>
                </div>
                <span class="weekly-pct-text">${child.weeklyPct}% (Weekly)</span>
            </div>
        `;
...
// Gamified Stats & Points System Calculation
function updateChildStats(child) {
    ...
    if (child.tasks.length === 0) {
        ...
        // Earn 10 points per chore done (capped at MAX_DAILY_PTS = 1000)
        child.dailyPoints = Math.min(completedCount * POINTS_PER_TASK, MAX_DAILY_PTS);
    }

    // Weekly calculation (Max 7,000 points)
    const activeDays = child.dailyProgress.filter(p => p > 0);
    if (activeDays.length > 0) {
        const totalPct = child.dailyProgress.reduce((sum, val) => sum + val, 0);
        child.weeklyPct = Math.round(totalPct / activeDays.length);
        
        // Sum total weekly points capped at MAX_WEEKLY_PTS
        const estimatedWeeklyDone = child.tasks.filter(t => t.completed).length * activeDays.length;
        child.weeklyPoints = Math.min(estimatedWeeklyDone * POINTS_PER_TASK, MAX_WEEKLY_PTS);
    } else {
        child.weeklyPct = 0;
        child.weeklyPoints = 0;
    }

    // Monthly calculation (Max 30,000 points)
    child.monthlyPoints = Math.min(child.weeklyPoints * 4, MAX_MONTHLY_PTS);
}

// Resets
window.resetDay = function(childId) {
    const child = familyData.find(c => c.id === childId);
    const todayIdx = getCurrentDayIndex();
    child.dailyProgress[todayIdx] = 0;
    child.tasks.forEach(t => t.completed = false);
...
window.resetWeekly = function(childId) {
    const child = familyData.find(c => c.id === childId);
    child.weeklyPct = 0;
...
// Leaderboard Modal & Winner Crowns
